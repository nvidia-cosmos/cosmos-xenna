...
_TEST_SETS = {
    "why_not_scaling_down": {
        "stage_params": [  # [cpus, gpus, throughput, num_workers_per_node]
            [0.25, 0, 0.67, 4],  # download
            [1, 0, 4000, None],  # remux
            [3, 0, 0.77, None],  # cpu decode
            [1, 0.25, 7.1 / 2, None],  # transnet
            [5, 0, 0.28, None],  # encoding
        ...
        "cluster_resources": {
            "num_nodes": 10,
            "num_cpus_per_node": 182,
            "num_gpus_per_node": 8,
        },
        "sequence_of_stage_ready": [5, 0],  # number of stages with measurements, 0 means all stages
    },
    "fragmentation_with_large_models": {
        "stage_params": [
            [0.25, 0, 0.45, 4],
            [1, 0, 4000, None],
            [1, 0.1, 0.42, None],  # gpu decoding
            [1, 0.25, 3.6 / 2, None],  # transnet
            [5, 0, 0.28, None],
            [3, 0, 0.082, None],
            [1, 8, 0.033, None],  # qwen-235b
            [1, 1, 0.98, None],  # t5
            [0.25, 0, 0.016, 8],
        ],
        "cluster_resources": {
    ...
    return resources.ClusterResources(
        nodes={
            f"node-{i}": resources.NodeResources(
                used_cpus=0,
                total_cpus=cluster_resources["num_cpus_per_node"],
                gpus=[
...
def _dump_worker_allocation(
    ...
    for idx, stage_spec in enumerate(pipeline_spec.stages):
        ...
        logger.info(f"Current {stage_name} has {len(worker_groups)} worker groups")
...
def run_autoscaling_test(test_name: str) -> None:
    ...
    for i in range(len(_TEST_SETS[test_name]["sequence_of_stage_ready"]) + 1):
        ...
        # add measurements
        if i == len(_TEST_SETS[test_name]["sequence_of_stage_ready"]):
            break
        num_stages_with_measurements = _TEST_SETS[test_name]["sequence_of_stage_ready"][i]
...
def test_autoscaling_algorithm() -> None:
    """Test the autoscaling algorithm.

    The pytest is just to make sure this code runs as it is useful for performance debugging.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test-name", "-t", type=str, choices=_TEST_SETS.keys(), default=next(iter(_TEST_SETS.keys())))
    args = parser.parse_args()
    run_autoscaling_test(args.test_name)
