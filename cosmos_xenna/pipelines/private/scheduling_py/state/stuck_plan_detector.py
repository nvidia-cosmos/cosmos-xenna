...
"""Stuck-plan WARN-to-INFO promotion and Prometheus emitter.

The Grow phase increments a per-stage stuck-plan counter every cycle
a positive-intent grow request goes partly unsatisfied (cluster
fragmentation, donor failure, hard-cap clamp). The detector promotes
the WARN-level latch into a one-shot INFO event when the counter
crosses ``threshold_cycles``, and emits the
``xenna_scheduler_stuck_plan_active`` gauge and
