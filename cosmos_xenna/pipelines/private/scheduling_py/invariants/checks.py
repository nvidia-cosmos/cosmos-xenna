...
violation indicates a scheduler defect (not an operator config
error): ``SchedulerInvariantError`` propagates out of
``autoscale`` so the corrupted plan is surfaced, not applied.
Each violation is logged at ERROR immediately before raising so
operators see the diagnostic even if a supervisor swallows the
outer exception.
"""
...
class PhaseBoundary(enum.StrEnum):
    ...
    Member ordering matches the per-cycle phase pipeline:
    ``MANUAL`` (operator overrides) -> ``FLOOR`` (floor enforcement)
    -> ``GROW`` (saturation-driven grow) -> ``SHRINK``
    (saturation-driven shrink) -> ``SOLUTION_DRAIN`` (finalizer
    drain into the rust planner solution).
...
def check_invariants_after_phase(
    *,
    ...
    Checks ``ctx.num_stages() == len(problem.stages)`` and that
    every stage's ``pending_add_count`` / ``pending_remove_count``
    is non-negative. Shape check runs first so a mismatch cannot
    leak through as ``IndexError`` mid-loop.

    Raises:
        SchedulerInvariantError: An invariant failed; the message
            names the phase and the offending stage / value.
...
def check_solution_shape(
    *,
    ...
    Solutions are consumed positionally; a length mismatch
    silently misroutes every staged add and remove, so this
...
def check_no_nan_in_classifier_state(
    *,
    phase_name: PhaseBoundary,
    stage_runtime_states: Mapping[str, StageRuntimeState],
) -> None:
    """Reject any per-stage EWMA value that is ``NaN`` or ``+/-Inf``.

    Validates ``slots_empty_ratio_ewma``,
    ``last_valid_slots_empty_ratio_ewma``, ``pressure_ewma``, and
    ``classifier_signal_noise_ewma`` on each entry; ``None`` is a
    valid cold-start sentinel.

    Raises:
        SchedulerInvariantError: A per-stage EWMA value is not
            finite. The message names the phase, stage, field,
            and value.

    """
    for stage_name, runtime in stage_runtime_states.items():
        for field_name, value in (
            ("slots_empty_ratio_ewma", runtime.classifier.slots_empty_ratio_ewma),
            ("last_valid_slots_empty_ratio_ewma", runtime.classifier.last_valid_slots_empty_ratio_ewma),
            ("pressure_ewma", runtime.pressure.ewma),
            ("classifier_signal_noise_ewma", runtime.classifier.signal_noise_ewma),
        ):
            if value is None:
                continue
...
def check_floor_after_shrink(
    *,
    ...
    pre_shrink_worker_counts: dict[int, int],
) -> None:
    """Validate Shrink did not reduce any stage below its floor or grow it.

    For every non-manual non-finished stage at index ``i``:
    ``min(pre_shrink_worker_counts[i], stage_floors[i]) <=
    current <= pre_shrink_worker_counts[i]``. The lower bound's
    ``min`` tolerates Floor grace-window states; the upper bound
    pins Shrink as remove-only.

    Raises:
        SchedulerInvariantError: A non-manual non-finished stage's
            post-Shrink worker count is outside the bounds above.
    ...
    for stage_index, problem_stage in enumerate(problem.rust.stages):
        ...
        try:
            ...
            pre_shrink = pre_shrink_worker_counts[stage_index]
        except (IndexError, KeyError) as exc:
            _log_and_raise_invariant(
                f"After {phase_name}: stage {problem_stage.name!r} (index "
                f"{stage_index}) cannot be resolved against the Shrink "
                f"collections (worker_ids_by_stage, stage_floors, "
                f"pre_shrink_worker_counts): {type(exc).__name__}: {exc!r}. "
                "Planner-state shape mismatch. This is a scheduler defect; "
                "report it with the autoscale cycle's problem_state."
            )
        floor_lower_bound = min(pre_shrink, floor)
        if current < floor_lower_bound:
            _log_and_raise_invariant(
                f"After {phase_name}: stage {problem_stage.name!r} "
                f"(index {stage_index}) has {current} workers but the "
                f"configured minimum-worker floor is {floor} (pre-Shrink was "
                f"{pre_shrink}). Shrink shrank the stage below the floor. "
                "This is a scheduler defect; report it with the autoscale "
                "cycle's problem_state."
            )
        if current > pre_shrink:
            _log_and_raise_invariant(
                f"After {phase_name}: stage {problem_stage.name!r} "
                f"(index {stage_index}) grew from {pre_shrink} workers to "
                f"{current}. Shrink must only remove workers. This is a "
...
def check_stuck_plan_monotonicity(
    *,
    ...
    Legal transitions per stage: ``curr == 0`` or ``curr == prev + 1``.
    Stages absent from ``prev_counters`` default to ``prev = 0``;
    stages absent from ``curr_counters`` are skipped.

    Raises:
        SchedulerInvariantError: A stage's transition is neither
            a reset nor a strict +1 increment.
