...
"""Cluster-wide ERROR formatter and Counter for absorbed allocation failures.

:func:`emit_allocation_failure` formats a per-GPU + per-node CPU
fragmentation snapshot, logs it at ``ERROR`` against ``loguru``, and
increments the per-stage Counter. It is generic and is consumed by
every phase that surfaces an absorbed allocation failure (the per-
phase ``AllocationFailureGate`` lives next to its only consumer in
``phases/grow/allocation_failure_gate.py``).

The snapshot helpers cover both the Python attrs ``GpuResources`` /
``NodeResources`` and the Rust binding via ``used_pool()`` /
``total_pool()`` so the same module works against either object
without runtime type checks leaking into the caller.
"""
...
_ALLOCATION_FAILURES_COUNTER = Counter(
    ...
    description=(
        "Per-stage scheduler-phase allocation failures absorbed by "
        "the skip_cycle_on_allocation_error gate. Increments once "
        "per raised AllocationError on any of Phase A (manual grow), "
        "Phase B (floor enforcement), or Phase C (saturation grow) "
        "before the phase short-circuits the cycle's remaining work."
    ),
