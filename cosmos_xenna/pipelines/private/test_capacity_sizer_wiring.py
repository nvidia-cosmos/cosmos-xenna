...
Pins the contract that :meth:`CapacityModel.target_workers`
populates ``StageRuntimeState.growth.capacity_target_workers``
so the classifier -> compute_delta path observes a non-cold-
start target once ``D_k`` becomes finite.
"""
...
def scheduler() -> SaturationAwareScheduler:
    ...
    state = sched.ledgers.stage_states["S"]
    state.classifier.resolved_thresholds = _resolve_auto_thresholds(
        stage_cfg=sched.pipeline.stage_config("S"),
        slots_per_actor=4,
    )
    return sched
...
class TestRefreshCapacityTargetWorkers:
    ...
    def test_returns_none_when_s_k_is_unobservable(self, scheduler: SaturationAwareScheduler) -> None:
        ...
        scheduler.ledgers.s_k_ewma.set("S", math.nan)
        capacity = CapacityModel(s_k_ewma=scheduler.ledgers.s_k_ewma)
        result = capacity.target_workers(
            stage_state=scheduler.ledgers.stage_states["S"],
            stage_cfg=scheduler.pipeline.stage_config("S"),
    ...
    def test_returns_none_when_thresholds_unresolved(self, scheduler: SaturationAwareScheduler) -> None:
        ...
        scheduler.ledgers.stage_states["S"].classifier.resolved_thresholds = None
        scheduler.ledgers.s_k_ewma.set("S", 5.0)
        capacity = CapacityModel(s_k_ewma=scheduler.ledgers.s_k_ewma)
        result = capacity.target_workers(
            stage_state=scheduler.ledgers.stage_states["S"],
            stage_cfg=scheduler.pipeline.stage_config("S"),
    ...
    def test_returns_target_when_s_k_finite(self, scheduler: SaturationAwareScheduler) -> None:
        ...
        scheduler.ledgers.s_k_ewma.set("S", 2.0)
        capacity = CapacityModel(s_k_ewma=scheduler.ledgers.s_k_ewma)
        result = capacity.target_workers(
            stage_state=scheduler.ledgers.stage_states["S"],
            stage_cfg=scheduler.pipeline.stage_config("S"),
