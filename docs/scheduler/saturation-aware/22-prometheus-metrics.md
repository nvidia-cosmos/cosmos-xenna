...
| Per-stage state | `xenna_stage_bottleneck_score` | gauge | `pipeline`, `stage` | Forced-Flow-Law bottleneck — see [23](23-bottleneck-score-metric.md). Per-stage `D_k = mean(end - start)` over the autoscale window; cold-start (no completed tasks yet) observes `NaN`. Wired off the same `update_with_measurements` path the backlog-time pressure throughput sample uses. The EWMA-smoothed copy of this gauge drives Phase C grow priority and Phase D shrink protection — see [25](25-bottleneck-decision-integration.md). |
| Per-stage state | `xenna_scheduler_cluster_heterogeneity_ratio` | gauge | `pipeline` | `max_k D_k / min_k D_k` across stages with finite service demand. `NaN` when fewer than two stages have observed a completed task this cycle. Streak-warned via `cluster_heterogeneity_warn_threshold` / `cluster_heterogeneity_warn_streak`. |
