...
ratio into one of **four operational zones** — `NORMAL`, `SATURATED`,
`SATURATED_CRITICAL`, `OVER_PROVISIONED` — with per-zone hysteresis
bands so the zone does not flap when the signal sits on a threshold
boundary. Four **discrete** zones, not a continuous controller,
because every action that follows (grow, shrink, no-op) gates on a
discrete state crossed for a streak of cycles.
...
Once the output is discrete, edge oscillation appears: the smoothed
signal drifts across a fixed threshold every other cycle, the state
flips every cycle, the streak counter resets every cycle, and no
decision is ever taken. Hysteresis on the exit edge is the standard
remedy.

## Decision

Adopt a **four-state classifier with per-zone asymmetric
...
Idle slots with a drained input queue resolve to `OVER_PROVISIONED`
under the same pressure-demotion gate. Whether the idleness is
genuine over-provisioning or backpressure-induced upstream of an
engaged bottleneck is captured at the **log layer** rather than as a
separate state — see
[27 — Topology-aware classifier](27-topology-aware-classifier.md).
...
```
 empty-slot ratio (EWMA smoothed); 0.0 = every slot busy, 1.0 = every slot free
   ...
   0.0      activation_thr   saturation_thr   over_provisioned_thr      1.0
    │             │                │                   │                 │
    ├─────────────┼────────────────┼───────────────────┼─────────────────┤
    │  SATURATED  │   SATURATED    │      NORMAL       │ OVER_PROVISIONED│
    │  _CRITICAL  │                │                   │                 │
    │             │                │                   │                 │
    └─────────────┴────────────────┴───────────────────┴─────────────────┘
       no
...
- **Four zones.** Phase C grow, Phase D shrink, and the streak gate
  each need a label they can count cycles against. A continuous
  controller would re-classify every cycle and oscillate around the
  decision boundary.
- **Per-zone asymmetric hysteresis.** Hysteresis is applied **only
  ...
  than scale-up.
- **Idle slots are uniformly `OVER_PROVISIONED` (subject to pressure-
  demotion).** Whether the input queue is empty or non-empty, the
  zone is the same; the pressure-demotion gate handles the
  "downstream stuck" case by demoting to `NORMAL` so a stage already
  paying the cost of a downstream bottleneck does not also lose
  workers and worsen the stall. Topology context (whether this
  stage is upstream or downstream of an engaged bottleneck) is
  surfaced on the per-cycle classifier-trace and classifier-
  transition log lines as `bottleneck_engaged=` and
  `upstream_of_bottleneck=` fields, not as a state-machine
  branch.
...
**pressure-demotion gate**: the smoothed pressure scalar must clear
the per-branch threshold for the slot-pin zone to be confirmed;
otherwise the zone falls through to the next rule.
...
| `ratio ≥ over_provisioned_boundary` AND `pressure_ewma > pressure_normal_threshold`                  | `NORMAL` (queue stuck downstream) |
| `ratio ≥ over_provisioned_boundary` (pressure low or unset)                                           | `OVER_PROVISIONED`                |
| otherwise                                                                                              | `NORMAL`                          |
...
falls back to the slot-only verdict (preserve the burst-response
...
| `over_provisioned_threshold`     | `0.50`        | Boundary between `NORMAL` and `OVER_PROVISIONED`                           |
...
- [27 — Topology-aware classifier](27-topology-aware-classifier.md)
  — why an idle drained queue maps uniformly to `OVER_PROVISIONED`,
  and how the per-cycle log lines surface the topology context for
  operators.
